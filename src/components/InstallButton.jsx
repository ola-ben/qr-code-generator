...
export default function InstallButton() {
  ...
  // Once the app is actually installed, there's nothing to offer.
  if (isInstalled) return null

  const handleClick = async () => {
    // Native prompt available (Android / desktop Chrome & Edge) → fire it.
    if (canPrompt) {
      const outcome = await promptInstall()
      // If the browser couldn't show it for some reason, fall back to the hint.
      if (outcome == null) setShowHelp((s) => !s)
      return
    }
    // iOS Safari or any browser that hasn't fired the prompt yet → show how-to.
    setShowHelp((s) => !s)
  }
    ...
    <div className="relative">
      <Tooltip label="Install QR Studio as an app" side="bottom">
        <button
          type="button"
          onClick={handleClick}
          aria-label="Install QR Studio as an app"
        ...
        <>
          ...
          <div className="absolute right-0 top-full z-40 mt-2 w-64 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-600 shadow-lg dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
            {isIos ? (
              ...
              <p>
                Open your browser menu <span className="font-semibold">(⋮)</span> and choose{' '}
